                        ...
                        INT16U   opt)
{
...
#if OS_ARG_CHK_EN > 0u
    if (prio > OS_LOWEST_PRIO) {             /* Make sure priority is within allowable range           */
        return (OS_ERR_PRIO_INVALID);        //如果比最低優先級(63)還大，表示創建太多task，return error
    }
#endif
    OS_ENTER_CRITICAL();
    if (OSIntNesting > 0u) {                 /* Make sure we don't create the task from within an ISR  */
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */   //檢查prioTable中該優先級的pointer是否為0(空)
        OSTCBPrioTbl[prio] = OS_TCB_RESERVED;/* Reserve the priority to prevent others from doing ...  */   //OS_TCB_RESERVED = 1，用1或0代表是否已有填值
                                             /* ... the same thing until task is created.              */
        OS_EXIT_CRITICAL();
        ...
        if (prio != OS_TASK_IDLE_PRIO) {                            //將task作delay，與OSTimeDly不同的是不重新作OS_Sched()
            ...
            TaskSchedInfo[id - 1].HoldR1 = 0;
            TaskSchedInfo[id - 1].HoldR2 = 0;
        ...
        if (err == OS_ERR_NONE) {
            ...
            OS_ENTER_CRITICAL();
            OSTCBPrioTbl[prio] = (OS_TCB *)0;                  /* Make this priority avail. to others  */   //沒創成功，把priority設成可使用
            OS_EXIT_CRITICAL();
        }
        return (err);
    }
